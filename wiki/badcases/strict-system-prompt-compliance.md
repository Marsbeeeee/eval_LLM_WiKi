...
0617 key guide + judge 验证说明，strict system prompt compliance badcase 可以通过 turn-level key guide 拆成 `[must]` 与 `[should]` 条款，再由 judge 逐项检查 evidence。业务关键失败应落在 `[must]`，例如工具参数缺失、固定结束语缺失或流程闭环缺失；体验质量问题可落在 `[should]`，例如未回应用户已提供的信息。来源：`raw/reports/[0617] key guide & judge 自动化尝试.docx`
